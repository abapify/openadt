...
- Verify block — `bunx eslint scripts/ .agents/skills/ --max-warnings 0 --no-error-on-unmatched-pattern` then `bun scripts/verify-spec-sync.ts` then `bun scripts/verify-package-docs.ts` then `./mvnw -q verify -Pdistribution -Dopenadt.distribution=true` then `bun run openadt:test`
