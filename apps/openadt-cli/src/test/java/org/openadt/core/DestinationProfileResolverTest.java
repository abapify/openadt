...
class DestinationProfileResolverTest {
    @Test
    void explicitSsoProfileOverlaysHttpTransportAndDiscoveryUrl() {
        OpenAdtConfig config = configWithDev(
            baseDestination(),
            profiles(Map.of(
                "sso",
                ssoProfile("https://dev-adt.example.com/sap/bc/adt")
            ))
        );
        ...
        assertEquals("https://dev-adt.example.com/sap/bc/adt", effective.getAdt().getDiscoveryUrl());
    ...
    void noProfileUsesDefaultProfile() {
        ...
        OpenAdtConfig config = configWithDev(
            destination,
            profiles(Map.of("sso", ssoProfile("https://dev-adt.example.com/sap/bc/adt")))
        );
  ...
  void resolveDoesNotMutateLoadedConfig() {
    ...
    private static SystemProfile.ProfileConfig ssoProfile(String discoveryUrl) {
        ...
        profile.setDiscoveryUrl(discoveryUrl);
