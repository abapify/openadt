...
| **Semgrep / Opengrep (repo config)** | This repo            | [`.semgrep.yaml`](../../.semgrep.yaml) at repo root (Codacy Opengrep + CI). No `codacy.*.hard-coded-password` rules (TOML key false positives).                     | CI job `opengrep`, `opengrep scan --config .semgrep.yaml`, or Security → Code scanning alerts.         | Whole-file `# nosemgrep` exclusions — use **line-specific** inline suppressions per [`.codacy/instructions/review.md`](../../.codacy/instructions/review.md). |
