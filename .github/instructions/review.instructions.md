...
| **GitHub Code Scanning**             | GitHub (GHAS)        | SARIF uploads from CodeQL, Semgrep Action, etc.                                                                                                                     | `gh api --paginate repos/{owner}/{repo}/code-scanning/alerts --jq '.[]                                 | select(.state=="open")'`                                                                                                                                      | Codacy, Dependabot    |
...
| **Dependabot**                       | GitHub               | Dependency vulnerability/version alerts.                                                                                                                            | `gh api --paginate repos/{owner}/{repo}/dependabot/alerts --jq '.[]                                    | select(.state=="open")'`                                                                                                                                      | Code Scanning, Codacy |
| **Semgrep / Opengrep (repo config)** | This repo            | Local rules in [`.semgrep.yml`](../../.semgrep.yml); CI runs [OpenGrep](https://github.com/opengrep/opengrep) and uploads SARIF to GitHub Code Scanning.            | CI job `opengrep`, `opengrep scan` locally, or Security → Code scanning alerts.                        | Whole-file `# nosemgrep` exclusions — use **line-specific** inline suppressions per [`.codacy/instructions/review.md`](../../.codacy/instructions/review.md). |
...
After `/act` P6, update the primary sink(s) per finding — do not duplicate the same lesson across multiple instruction files.
